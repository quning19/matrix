...
class TexasRules():
    ...
    def get_best_match(self, cards):
        ...
        for match_type in reversed(list(TexasMatch)):  # 从 STRAIGHT_FLUSH 到 HIGH_CARD
            match, ranks = self.rules[match_type](cards)
            if match:
                print(match, ranks)
                return match_type, ranks
    ...
    def check_one_pair(self, cards):
        ...
        pair_cards, left = self.split_match_rank(cards, pairs[0])
    ...
    def check_two_pairs(self, cards):
        ...
        pairs.sort(reverse=True)
        pair_cards_1, left = self.split_match_rank(cards, pairs[0])
        pair_cards_2, left = self.split_match_rank(left, pairs[1])
    ...
    def check_three_of_a_kind(self, cards):
        ...
        trip_cards, left = self.split_match_rank(cards, trips[0])
    ...
    def check_straight(self, cards):
        ...
        for i in range(straight_top, straight_top - 5, -1):
            ...
            matches, cards = self.split_match_rank(cards, i)
    ...
    def check_flush(self, cards):
        ...
        flush_cards, left = self.split_match_suit(cards, flush[0])
    ...
    def check_full_house(self, cards):
        ...
        counts = Counter([card.rank for card in left])
        pairs = [k for k, v in counts.items() if v >= 2]
    ...
    def check_four_of_a_kind(self, cards):
        ...
        quads_cards, left = self.split_match_rank(cards, quads[0])
        _, left = self.check_high_card(left, 1)
    ...
    def split_match_rank(self, cards, rank):
        matches = [card for card in cards if card.rank == rank]
        left = [card for card in cards if card.rank != rank]
        left.sort(key=lambda card: card.rank, reverse=True)
        return matches, left
    
    def split_match_suit(self, cards, suit):
        matches = [card for card in cards if card.suit == suit]
        matches.sort(key=lambda card: card.rank, reverse=True)
        left = [card for card in cards if card.suit != suit]
        left.sort(key=lambda card: card.rank, reverse=True)
        return matches, left
